...
- 动漫风背景：内置 6 张 Pixabay 动漫风花卉/风景插画，右下角按钮一键切换，圆环扩散 + 奶油淡出收尾转场，选择记忆在浏览器
- 季节天气开场：每次打开网站先展示约 5.5 秒的分幕式「季节 + 当地天气」开场动画（大字季节 → 天气实景特效 → 天气卡片 → 柔和淡出；大雨飘雨、下雪飘雪、多云飘云、雾天起雾、雷暴闪雷；IP 定位 + Open-Meteo 免费接口），单击/双击/空格均可跳过，断网自动降级
